...
function MailSend() {
    ...
    const [data, setData] = useState({ sendMailList: [] });
    ...
    const filteredList = data?.sendMailList?.filter(sml => {
        const matchesSearch =
            sml.mail_name.includes(searchTerm) ||      // 제목 검색
            sml.receiver_name.includes(searchTerm) ||   // 보낸 사람 검색
            sml.mail_receiver.includes(searchTerm);     // 이메일 주소 검색

        if (!matchesSearch) return false;

        if (checked === "") return true;
        if (checked === "read") return sml.mail_sread === 0;
        if (checked === "imp") return sml.mail_simp == 1;
        if (checked === "lock") return sml.mail_slock == 1;
        return true;
    });
    ...
    async function getMailSendList(page = 1, filter = "", keyword = "") {
        ...
        try {
            let response = await getMailSend(page, filter, keyword);
    ...
    const handleReset = () => {
        ...
        navigate(`/mail/send?memId=${user.mem_id}`); // URL 초기화

        setChecked("");               // 라디오 버튼 초기화
        setBoxChecked([]);            // 선택된 체크박스 초기화
        getMailSendList(1);        // 데이터 다시 불러오기
        setSearchTerm("");
    };
    ...
    const handleSelectAll = () => {
        if (!data?.sendMailList) return;

        if (boxChecked.length === data.sendMailList.length) {
            setBoxChecked([]);
        } else {
            const allIds = data.sendMailList.map((sml) => sml.mail_id);
    ...
    const handleToggleLock = async (mail_id) => {
        ...
        setData(prev => {
            const updated = { ...prev };
            updated.sendMailList = updated.sendMailList.map(mail =>
                mail.mail_id === mail_id ? { ...mail, mail_slock: mail.mail_slock === 0 ? 1 : 0 } : mail);
            return updated;
        });

        try {
            const res = await updateMailSnedLockToggle(mail_id);
    ...
    const handleToggleImp = async (mail_id) => {
        ...
        setData(prev => {
            ...
            updated.sendMailList = updated.sendMailList.map(mail => {
                if (mail.mail_id === mail_id) {
                    prevSimp = mail.mail_simp; // 이전 값 저장
                    return { ...mail, mail_simp: mail.mail_simp === 0 ? 1 : 0 };
                }
                return mail;
            });
            return updated;
        ...
        try {
            const res = await updateMailSendImpToggle(mail_id);
            ...
            if (!res.data.success) {
                ...
                setData(prev => {
                    const updated = { ...prev };
                    updated.sendMailList = updated.sendMailList.map(mail =>
                        mail.mail_id === mail_id ? { ...mail, mail_simp: prevSimp } : mail);
                    return updated;
                });
                // alert("잠금 상태 변경 실패");
            }
    ...
    const handleWaste = async () => {
        ...
        const mailsToDelete = data.sendMailList
            .filter(mail => boxChecked.includes(mail.mail_id) && mail.mail_slock === 0)
                ...
                <ListHeader style={{ height: '97px', padding: '13px 25px' }}>
                    ...
                    <FlexDiv style={{ marginTop: '10px' }}>
                        <FlexDiv style={{ width: '150px', justifyContent: 'start', gap: '8px' }}>
                            <BoxButton style={{ cursor: 'pointer' }} onClick={handleSelectAll}></BoxButton>
                            <BoxButton style={{ backgroundImage: `url(${trash})`, cursor: 'pointer' }} onClick={handleWaste}></BoxButton>
                            <BoxButton style={{ backgroundImage: `url(${replace})`, cursor: 'pointer' }} onClick={handleReset}></BoxButton>
                        </FlexDiv>
                        <FlexDiv style={{ width: '280px', justifyContent: 'end', marginTop: '3px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', overflowY: 'auto' }}>
                    {loading ? null : (
                        filteredList.length === 0 ? (
                            ...
                            filteredList.map((sml) =>
                                <WHContainer key={sml.mail_id}>
                                    <MailView>
                                        <img src={sml.mail_slock === 0 ? unLock : lock} style={{ height: '20px', marginTop: '8px' }} onClick={() => handleToggleLock(sml.mail_id)}></img>
                                    </MailView>

                                    <div>
                                        <FlexDiv>
                                            <label style={{ display: "flex", justifyContent: 'end', marginLeft: '10px' }}>
                                                <CheckBox type='checkbox' name="check" value={sml.mail_id} checked={boxChecked.includes(sml.mail_id)} onChange={handleCheckboxChange} />
                                                <CheckMark />
                                            </label>
                                            <Link to={`/mail/detail/${sml.mail_id}?memId=${user.mem_id}`} style={{ width: '240px' }}>
                                                <FlexDiv>
                                                    <MailTitle style={{ marginLeft: '10px', marginTop: '-2px' }}>{sml.receiver_name}</MailTitle>
                                                    <DateBox style={{ marginLeft: '10px' }}>{sml.mail_receiver}</DateBox>
                                                </FlexDiv>
                                                <FlexDiv>
                                                    <MailTitle style={{ maxWidth: '223px', marginLeft: '10px', marginTop: '3px', fontWeight: '400' }}>{sml.mail_name}</MailTitle>
                                                    {sml.mailFileList.length > 0 && (
                                    ...
                                    <div style={{ marginLeft: 'auto', marginTop: '-2px' }}>
                                        <DateBox>{formatDate(sml.mail_sdate)}</DateBox>
                                        <div>
                                            <img src={sml.mail_simp === 0 ? unImp : imp} style={{ width: '18px', height: '18px', marginLeft: '32px', cursor: 'pointer' }} onClick={() => handleToggleImp(sml.mail_id)}></img>
                                        </div>
                                    </div>
                                </WHContainer>
                            )
